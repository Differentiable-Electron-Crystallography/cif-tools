...
### Why These Files Are Required (Maturin Mixed Layout)

This project uses maturin's **mixed Rust/Python layout**, which combines compiled Rust code with pure Python source files. Understanding this layout explains why several files must be manually maintained.
...
- **`python-source = "src"`** - Tells maturin to look for Python source code in `python/src/`
- **`module-name = "cif_parser._cif_parser"`** - The compiled Rust extension is named `_cif_parser` (underscore prefix indicates a private module)

**Package Structure:**
```
python/src/cif_parser/
├── __init__.py              ← Public Python interface (manually maintained)
├── __init__.pyi             ← Type stubs for the public API (manually maintained)
├── _cif_parser.cpython-*.so ← Compiled Rust extension (maturin generates this)
├── _cif_parser.pyi          ← Type stubs for native module (manually maintained)
└── py.typed                 ← PEP 561 marker file (manually maintained)
```

**What Maturin Generates vs. What You Maintain:**

| File | Generated by Maturin? | Purpose |
|------|----------------------|---------|
| `_cif_parser.*.so` | ✅ Yes | Compiled Rust extension |
| `__init__.py` | ❌ No (mixed projects) | Public API, re-exports from Rust |
| `__init__.pyi` | ❌ No | Type hints for public API |
| `_cif_parser.pyi` | ❌ No | Type hints for native module |
| `py.typed` | ❌ No (mixed projects) | PEP 561 marker |

For **pure Rust projects**, maturin auto-generates `__init__.py` and includes `py.typed`. For **mixed projects** (like this one), maturin does not modify existing Python files—you maintain full control.

#### `__init__.py` - Public Interface

The `__init__.py` serves as the public interface for your package:

```python
from ._cif_parser import (
    Document, Block, Loop, Frame, Value,
    parse, parse_file, __version__
)

__all__ = ["Document", "Block", "Loop", "Frame", "Value", "parse", "parse_file"]
```

**Why it's needed:**
1. **Clean Public API** - Users import from `cif_parser`, not `cif_parser._cif_parser`
2. **Documentation** - Module-level docstrings and `__all__` for discoverability
3. **Flexibility** - You can add pure Python code, re-export selectively, or provide wrappers

**Without `__init__.py`:**
```python
# Ugly - exposes internal structure:
from cif_parser._cif_parser import Document, parse
```

**With `__init__.py`:**
```python
# Clean, Pythonic:
from cif_parser import Document, parse
```

The `python-source` option is also recommended by maturin to "avoid a common `ImportError` pitfall" where Python might import from source instead of the compiled extension during development.

#### `.pyi` Files - Type Stubs

Type stub files (`.pyi`) provide static type information for type checkers like mypy and pyright, and enable IDE features like autocomplete and hover documentation.

**`__init__.pyi`** - Type hints for the public API:
- Documents all exported classes (`Document`, `Block`, `Loop`, `Frame`, `Value`)
- Documents all functions (`parse`, `parse_file`)
- Includes docstrings that appear in IDE hover tooltips
- More comprehensive documentation than `_cif_parser.pyi`

**`_cif_parser.pyi`** - Type hints for the native Rust module:
- Mirrors the Rust-defined classes and functions
- More concise, focuses on signatures
- Used when directly importing from `_cif_parser` (not recommended)

**Why maturin doesn't generate `.pyi` files:**
PyO3/maturin cannot automatically infer Python type annotations from Rust code. The type information must be manually written to accurately describe the Python API. This is a known limitation—the Rust type system doesn't map 1:1 to Python's type hints.

**Example type stub usage:**
```python
# In __init__.pyi
class Value:
    @property
...
This enables type checkers to understand:
```python
value = block.get_item("_cell_length_a")
if value is not None and value.is_numeric:
    length = value.numeric  # Type checker knows: float | None
```

#### `py.typed` - PEP 561 Marker

The `py.typed` file is a marker file defined by [PEP 561](https://peps.python.org/pep-0561/) that signals to type checkers that this package contains type information.

**Contents:**
```
# Marker file for PEP 561 - this package contains type information
```

**What it does:**
- Tells mypy, pyright, and other type checkers to use the `.pyi` files in this package
- Without it, type checkers may ignore your type stubs
- Must be present in the package directory (not just the project root)

**Why maturin doesn't generate it for mixed projects:**
In pure Rust projects, maturin includes `py.typed` automatically when you provide a `.pyi` stub. In mixed projects, maturin doesn't modify your Python source directory, so you must create it yourself.
